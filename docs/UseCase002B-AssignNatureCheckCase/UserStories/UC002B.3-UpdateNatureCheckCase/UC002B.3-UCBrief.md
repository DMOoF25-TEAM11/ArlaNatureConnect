...
The Arla employee updates an existing active Nature Check Case for a farm with new consultant, priority, and notes. The system validates the update and updates the case information. Notifications are regenerated from updated case data when consultant views them (not stored in database).
...
3. If consultant changed, notifications are regenerated from updated case data when consultant views them (not stored in database).
...
10. The system updates the assignment timestamp (AssignedAt).
