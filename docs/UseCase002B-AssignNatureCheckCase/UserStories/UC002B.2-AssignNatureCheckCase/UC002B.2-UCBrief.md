...
The Arla employee creates a new Nature Check Case for a selected farm and assigns it to a consultant with optional priority level and notes. The consultant receives a database notification about the new assignment.
...
3. The consultant receives a notification about the new assignment.
...
10. The system creates a database notification for the consultant.
11. The system updates the farm list to show "Tilføjet" status.
...
### 10a. Notification creation fails:
...
- Notifications: Database-based (consultant sees notification badge in UI), not email/SMS.
