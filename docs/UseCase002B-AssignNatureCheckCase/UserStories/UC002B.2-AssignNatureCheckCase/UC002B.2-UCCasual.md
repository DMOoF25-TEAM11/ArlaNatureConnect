...
- Notifications are available when consultant views them (generated from case data, not stored in database).  
...
12. Notifications are generated from case data when consultant views them (not stored in database).  
...
- **11a. No cases found for consultant:**  
...
- Notifications are generated from NatureCheckCase data when consultant views them (not stored in database). ConsultantNotificationDto objects are created in-memory from case data.
