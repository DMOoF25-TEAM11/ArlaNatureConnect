...
│    Metode: AddAsync() (linje 44, arver fra base)            │
│    ↓ bruger                                                 │
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
...
## 🧪 Dybdegående Gennemgang af Tests for UC002 Flow

### Test Setup (Før alle tests)
...
public async Task AddCommand_Creates_Address_And_Person_And_Resets_Form()
{
    ...
    FakePersonRepo personRepo = new();      // Fake repository (ikke mock)
    FakeAddressRepo addrRepo = new();       // Fake repository (ikke mock)
    FakeRoleRepo roleRepo = new();          // Fake repository (ikke mock)
    ...
    // ASSERT
    Assert.IsTrue(personRepo.Store.Count > 0, "Person repository should have an added person");
    Assert.IsNotEmpty(addrRepo.Store, "Address repository should have an added address");
    Assert.IsTrue(vm.FirstName == string.Empty && vm.IsAddMode, "Form should be reset after add completes");
}
```

**Hvad er Fake Repositories?**
- I stedet for Mocks (Moq), bruger denne test "Fake" repositories
...
- ✅ AddCommand kan udføres når form er gyldig
- ✅ Address bliver oprettet først
